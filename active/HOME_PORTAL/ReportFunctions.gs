...
function buildAbsenReportCacheKey(nik, deptFilter, periodType, periodValue) {
  return ['absen', asText(nik).trim()||'-', asText(deptFilter).trim()||'-', asText(periodType).trim()||'-', asText(periodValue).trim()||'-'].join(':');
}
...
function getAbsenReport(nik, deptFilter, periodType, periodValue) {
  try {
    ...
    if (lastRow <= 1) {
      const empty = { ok: true, period: range.label, total: 0, complete: 0, active: 0, data: [] };
      cache.put(cacheKey, JSON.stringify(empty), 90);
      return empty;
    }
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      rows.push({
        tanggal: asText(row[0]), nik: rowNik, nama: asText(row[2]), dept: rowDept,
        jabatan: asText(row[4]), jamMasuk: asText(row[5]), jamKeluar: asText(row[6]),
        status, noKartuMK: asText(row[8]), noLoker: asText(row[9]),
        sortKey: dateKey + '|' + asText(row[5] || '')
      });
    }

    rows.sort((a, b) => a.sortKey === b.sortKey ? a.jamMasuk.localeCompare(b.jamMasuk) : a.sortKey.localeCompare(b.sortKey));
    rows.forEach(r => delete r.sortKey);

    const result = { ok: true, period: range.label, total: rows.length, complete, active, data: rows };
...
function getAreaActivityReport(nik, deptFilter, periodType, periodValue) {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      rows.push({
        noKartuMK: normalizeCard(data[i][0]), inout,
        tanggal:   asText(data[i][2]),        jam:     asText(data[i][3]),
        nik:       rowNik,                    nama:    asText(data[i][5]) || asText(employee.nama),
        dept:      asText(employee.dept),     jabatan: asText(employee.jabatan),
    ...
    return { ok: true, period: range.label, total: rows.length, inCount, outCount, data: rows };
  } catch(e) {
    return { ok: false, msg: e.message };
  }
}
